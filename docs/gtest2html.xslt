				...
				<xsl:when test="*">
					<td id="failed">
						<xsl:for-each select="failure">
							<xsl:if test="@message">
								<xsl:call-template name="replace-breakline"><xsl:with-param name="text" select="@message" /></xsl:call-template>
							</xsl:if>
						</xsl:for-each>
					</td>
...
<xsl:template name="replace-breakline">
		<xsl:param name="text" />
		<xsl:choose>
			<xsl:when test="contains($text, '&#x0A;')">
				<xsl:value-of select="substring-before($text,'&#x0A;')" /><br/> 
				<xsl:call-template name="replace-breakline">
